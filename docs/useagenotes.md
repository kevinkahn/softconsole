# Installation
* The system has currently been tested on Raspberry Pi Zero, 2, and 3 using an Adafruit 3.5" resistive PiTFT and Adafruit 2.8" capacitive PiTFT.  To set up a system use one of the following methods:
    * Easiest: Build your Pi using one of the prebuilt images from Adafruit that is appropriate to the display that you are using.  Before booting the Pi add the file earlyprep.sh to the /boot partition on whatever system you built the SD card on.  Then after booting run as root "bash /boot/earlyprep.sh" which will prompt you to configure WiFi if needed, expand the file system, and set the WiFi country.  Reboot when the script exits and after the reboot run the piprepadafruit.sh script as root (bash ./piprepadafruit.sh).  This script takes no parameters but asks a series of questions to decide on the configuration details.    After the prep script completes it may leave a TODO file in the pi home directory of additional configuration things that are needed.
    * Alternative: Build your Pi using the most recent version of Jessie from the Raspberry Pi Foundation.  Proceed as above but run the script piprep.sh rather than piprepadafruit.  This version will ask what screen type you are using.  Recently I have seen an issue with this way of configuring the OS that leaves the touch functions wrong at least on the 2.8 screens.  I'm trying to sort out what the issue is with this.
    * Expert alternative: Configure its networking and expand its file system.  Manually examine the piprep.sh script to perform the specific configurations and installations that make sense for your system.
