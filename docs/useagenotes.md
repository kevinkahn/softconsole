# Installation
* The system has currently been tested on Raspberry Pi Zero, Pi Zero W, 2, and 3 using an Adafruit 3.5" resistive PiTFT and Adafruit 2.8" capacitive PiTFT.  Some limited testing has been done on Chinese clone screens - no guarantees there as you may have to work on their calibrations.  To set up a system use one of the following methods:
    * Easiest:  Build your Pi using the a recent Raspbian image from the Foundation.  The system has been tested and run on Jessie and Stretch versions.  (As of 8/29/2017 the current version is a Stretch version.)  Add the **pisetup.sh** script to the /boot partition on the SD card while you have the card in whatever system you use to write the image.  After booting the Pi with this image you may want to configure WiFi from the console if you need that.  Then run as root `bash /boot/pisetup.sh`.  
        * Note: Various subscripts get run by pisetup some of which give the option of rebooting the system.  *DO NOT REBOOT* from these subscripts.  You should manually reboot only after the entire pisetup script finishes.

        This script will ask questions about things you might want configured into the system (they are things I use in some of my consoles.)  You probably only want to supply a node name and install TightVNC.  The script will also launch respi-config to setup localization.  If you are using WiFi you should set the country.  You should also set the time zone.  Late in the script it will ask you if you want to install the re4son kernel - answer Y.
          
         When this script exits it asks you to reboot and then run **installconsole.sh** as root after the reboot.  This script will ask you what screen you are using.  The common ones are the Adafruit PiTFTs which are designated 35r, 28r, and 28c.  However the is support for other screens that I have not tested.  You can see a list of all the screens that have at least kernel support listed just before the prompt to supply the screen type or later after the installs finish by running:
         ```
         sudo /usr/local/src/re4*/re4*setup -h
         ``` 
      After the script completes it may leave a TODO file in the pi home directory of additional configuration things that are needed.  
        * Note: There are some long pauses in my experience doing this.  When you first boot Stretch, e.g., it may seem to give you a prompt and then blank the screen.  It stays blank a long time (3-4 minutes) and then opens a graphical console.  There is also a long download that is likely to be slow because the source is in Australia.  There are also long pauses when the pisetup script gets to updating the system to add Bluetooth and when the installconsole script has to downgrade the touchscreen stuff.
...
To insure that the basic setup of the Console is ok and that you understand the pieces, there is a minimal test configuration that you might want to run.  In example configs is a config file names minimaltest.txt.  It defines one key screen with a single large key and a clock screen that should appear when idle.  Move this file to /home/pi/Console.  In example configs subdirectory cfglib find auth.cfg and myclock.cfg and move them to /home/pi/Console/cfglib.  Edit the auth.cfg to point at your ISY IP address and have your name and password.  Rename minimaltest.txt to config.txt and edit it to replace "Office Ceiling" with the name of some switch in your ISY configuration that you'll control for the test.  
...
Given an understanding for the minimal test above you can then create real configuration files as you wish:
* Create a config.txt file, see the files in the "example configs" directory within consolestable for help.  The basic structure of the file is a sequence of sections started with \[section name] where the outermost part of the file is implicitly a section.  Subsections which are currently only used to describe keys are started within a section with \[\[subsection]].  Within any section are parameter assignments of the form name = value.  A complete list of current parameters is found in the params.txt file in this directory.  It lists the global parameters with their type and default value if no assignment is supplied.  It also lists for each module the local parameters of that module as well as any global parameters that can be overridden in that module.  Strings may be written without quotes.  
