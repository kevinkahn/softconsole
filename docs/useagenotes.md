...
* The system has currently been tested on Raspberry Pi Zero, Pi Zero W, 2, and 3 using an Adafruit 3.5" resistive PiTFT and Adafruit 2.8" capacitive PiTFT.  Some limited testing has been done on Chinese clone screens - no guarantees there as you may have to work on their calibrations.  To set up a system use one of the following methods:
    * Most tested:  Build your Pi using the a recent Raspbian image from the Foundation.  The system has been tested and run on Jessie and Stretch versions.  (As of 8/29/2017 the current version is a Stretch version.)  Add the "pisetup.sh" script to the /boot partition on the SD card while you have the card in whatever system you use to write the image.  After booting the Pi with this image you may want to configure WiFi from the console if you need that.  Then run as root "bash /boot/pisetup.sh".  This script will ask questions about things you might want configured into the system (they are things I use in some of my consoles.)  You probably only want to supply a node name and install TightVNC.  The script will also launch respi-config to setup localization.  If you are using WiFi you should set the country.  You should also set the time zone.  When this script exits it asks you to reboot and then run installconsole.sh as root after the reboot.  After the script completes it may leave a TODO file in the pi home directory of additional configuration things that are needed.
