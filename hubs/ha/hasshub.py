...
class HA(object):
	...
	def HAevents(self):
		...
		def on_message(qws, message):
			self.evhist.append((time.strftime('%m-%d-%y %H:%M:%S', time.localtime(time.time())), f'Message: {message}'))
			prog = 0
			loopstart = time.time()
		...
		def on_error(qws, error):
			self.evhist.append((time.strftime('%m-%d-%y %H:%M:%S', time.localtime(time.time())), f'Error: {error}'))
		...
		def on_close(qws, code, reason):
			self.evhist.append(
				(time.strftime('%m-%d-%y %H:%M:%S', time.localtime(time.time())), f'Close: {code} {reason}'))
			self.HB.Entry('Close')
		...
		def on_open(qws):
			...
			self.evhist.append((time.strftime('%m-%d-%y %H:%M:%S', time.localtime(time.time())), f'Open'))
			self.HB.Entry('Open')
			if self.restarting:
		...
		self.evhist = deque(maxlen=100)
		self.evhist.append((time.strftime('%m-%d-%y %H:%M:%S', time.localtime(time.time())), 'Creating WS'))
		# import logging
		# code:
		...
		logsupport.Logs.Log(self.name + " Event Thread " + str(self.HAnum) + " exiting", severity=sev,
		                    tb=False)
		# if utilfuncs.ishomesystem:
		if utilfuncs.ishomesystem or utilfuncs.isdevsystem:
			with open(f'/home/pi/Console/.HistoryBuffer/{self.name}-wsdump', 'a') as f:
				print(f'WS Dump Start thread {self.HAnum}', file=f)
				while True:
					try:
						print(self.evhist.popleft(), file=f)
					except IndexError:
						break
				print(f'---------------- end: {self.HAnum}', file=f)
