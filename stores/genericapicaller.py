...
//a.com/xyz?key={},lat={}
where in the {} go other store items? or items from config file so like for weather where I have city= then that would
sub into
{city}

maybe simpler to have a loadable json getter that takes a set of keyword parameter, calls an api, parses the json and
 returns a dict
...
in the watching thread need to do a PostEvent(ConsoleEvent(CEvent.SchedEvent, **self.kwargs)) with the proc = alert.
Invoke when the file changes, otherwise the watching thread just processes the var changes and goes back to watching
